...
{    
    ...
    {    
        ...
        template<typename T>
        GLB_INLINE constexpr mat<3, 3, T> ToMat3x3(const quat<T>& q) noexcept
        {
            return Mat::FromQuat_3x3(q);
        }

        template<typename T>
        GLB_INLINE constexpr mat<4, 4, T> ToMat4x4(const quat<T>& q) noexcept
        {
        ...
        template<typename T>
        GLB_INLINE constexpr quat<T> FromMat(const mat<3, 3, T>& m) noexcept
        {
        ...
        template<typename T>
        GLB_INLINE constexpr quat<T> FromMat(const mat<4, 4, T>& m) noexcept
        {
